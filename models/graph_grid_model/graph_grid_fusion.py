...
class Graph2Grid(nn.Module):
    ...
    def forward(self, graph_data, grid_data, lat_lon_coords, graph_time_indices, grid_time_indices):
        ...
        interp_grid = self.graph_to_grid_interp(graph_data, lat_lon_coords, H, W).permute(0, 1, 3, 4, 2)
    ...
    def graph_to_grid_interp(self, graph_data, lat_lon_coords, H, W, sigma=0.1, eps=1e-6):
        """
        从graph数据插值到grid格式
        ...
        """
        B, T, N, C = graph_data.shape
        device = graph_data.device

        # 创建网格坐标 [H*W, 2]，值域为0~1
        ys = torch.linspace(0, 1, H, device=device, dtype=graph_data.dtype)
        xs = torch.linspace(0, 1, W, device=device, dtype=graph_data.dtype)
        grid_y, grid_x = torch.meshgrid(ys, xs, indexing='ij')  # [H, W]
        grid_coords = torch.stack([grid_y, grid_x], dim=-1).view(-1, 2)  # [H*W, 2]

        all_interp = []

        for b in range(B):
            # 当前样本的坐标： [N, 2]
            coords_b = lat_lon_coords[b]  # [N, 2]

            # 距离和权重计算
            dists = torch.cdist(grid_coords, coords_b, p=2)  # [H*W, N]
            weights = torch.exp(-dists ** 2 / (2 * sigma ** 2))  # [H*W, N]
            weights = weights / (weights.sum(dim=-1, keepdim=True) + eps)  # [H*W, N]

            # 当前样本图数据: [T, N, C]
            data_b = graph_data[b]  # [T, N, C]
            data_b = data_b.reshape(T, N, C)

            # 插值：weights: [H*W, N] @ data_b: [T, N, C] -> [T, H*W, C]
            interp_b = torch.matmul(weights, data_b)  # [T, H*W, C]
            interp_b = interp_b.view(T, H, W, C).permute(0, 3, 1, 2)  # [T, C, H, W]
            all_interp.append(interp_b)

        result = torch.stack(all_interp, dim=0)  # [B, T, C, H, W]
        return result
...
class Grid2Graph(nn.Module):
    def __init__(self, patch_size, grid_channels, graph_channel, graph_nodes, node_features):
        ...
        self.gnn = SimpleMLP(input_dim=node_features, hidden_dim=node_features * 2, output_dim=node_features)
    ...
    def forward(self, grid_data, graph_data, lat_lon_coords, graph_time_indices, grid_time_indices):
        ...
        for b in range(batch_size):
            ...
            for t_idx in range(graph_time_indices.size(1)):
                ...
                x = self.gnn(combined_features.squeeze(0))  # Shape: [num_combined_nodes, feature_dim]
...
class GraphGridCrossAttnModel(nn.Module):
    ...
    def forward(self, graph_features, grid_features, graph_time_indices, grid_time_indices):
        ...
        graph_time_indices = self.grid_time_emb(graph_time_indices)

        graph_features = rearrange(graph_features, 'b t n c -> b (t n) c')
        graph_attn_output = self.cross_attn_graph(graph_time_indices, graph_features, graph_features)  # (batchsize, t *, d)
        graph_attn_output = rearrange(graph_attn_output, 'b (t n) c -> b t n c',t=t,n=n)

        grid_features_patched = rearrange(self.patch_emb(grid_features), 'b t n c -> b (t n) c')
        grid_time_indices = self.grid_time_emb(grid_time_indices)
        grid_attn_output = self.cross_attn_grid(grid_time_indices, grid_features_patched, grid_features_patched)  # (batchsize, t, d)
